...
class EGL(object):
    ...
    def _show_shader_log(self, shader):
        ...
        log=(ctypes.c_char*N)()
        loglen=ctypes.c_int()
        glGetShaderInfoLog(shader,N,ctypes.byref(loglen),ctypes.byref(log))
        print log.value
    ...
    def _show_program_log(self, program):
        ...
        log=(ctypes.c_char*N)()
        loglen=ctypes.c_int()
        glGetProgramInfoLog(program,N,ctypes.byref(loglen),ctypes.byref(log))
        print log.value
    ...
    def load_shader ( self, shader_src, shader_type = GL_VERTEX_SHADER, quiet = True ):
        ...
        glShaderSource(shader, 1, ctypes.byref(c_shader_src), 0)
